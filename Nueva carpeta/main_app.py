...
class Home(ctk.CTk):
    ...
    def button_event_agg(self):
            ...
            self.opcion_phone = ctk.CTkOptionMenu(self.main_frame, values=["0424","0414","0412","0416","0426"])
            self.opcion_phone.grid(row=3,column=4, padx=20, pady=20)
            self.opcion_phone.place(x=600, y=165)
            self.entry_phone = ctk.CTkEntry(self.main_frame, placeholder_text="Num. De Teléfono", font=('Century Gothic', 16), width=150)
            self.entry_phone.grid(row=3, column=5, padx=10, pady=10)
            self.entry_phone.place(x=750, y=165)
            self.button_next = ctk.CTkButton(master=self.main_frame, text="Siguiente", command=self.button_event_next)
            self.button_next.grid(row=4, column =4, padx=10, pady=10)

    def button_event_next(self):
        tipo_doc = self.opcion_menu_ID.get()
        nombre = self.entry_user_name.get()
        apellido = self.entry_user_lastName.get()
        cedula = self.entry_ID.get()
        direccion = self.textbox.get("1.0", tk.END)
        phone_entry = self.entry_phone.get()
        phone_opction = self.opcion_phone.get()

        telefono = f"{phone_opction}{phone_entry}"

        # Validar que se ingresen todos los campos requeridos
        if not cedula or not nombre or not apellido or not telefono or not direccion or not tipo_doc:
        ...
        try:
            ...
            c = conn.cursor()
            c.execute("INSERT INTO clientes (cedula, nombre, apellido, telefono, direccion, tipo_doc) VALUES (?, ?, ?, ?, ?, ?)",
                      (cedula, nombre, apellido, telefono, direccion, tipo_doc))
            conn.commit()

            tkinter.messagebox.showinfo("Éxito", "El cliente se ha creado exitosamente")
            # Cerrar la conexión a la base de datos
            c.close()
            conn.close()
        except Exception as e:
            tkinter.messagebox.showerror("Error", f"No se pudo crear el cliente: {e}")
    ...
    def button_event_agg_products(self):
        ...
        self.entry_name_products = ctk.CTkEntry(master=self.main_frame, placeholder_text="Tornillo 4/3", font=('Century Gothic', 16))
        ...
        self.entry_code_products = ctk.CTkEntry(master=self.main_frame, placeholder_text="B3DF352GH", font=('Century Gothic', 16))
        ...
        self.textbox_des = ctk.CTkTextbox(self.main_frame, width=200, height=100)
        self.textbox_des.grid(row=2, column=2, padx=10, pady=10)
        self.label_cant = ctk.CTkLabel(master=self.main_frame, text="Cantidad de Productos:", font=('Century Gothic', 16))
        self.label_cant.grid(row=3, column=2, padx=20, pady=20)
        self.entry_cant = ctk.CTkEntry(master=self.main_frame, placeholder_text="Cantidad", font=('Century Gothic', 16))
        self.entry_cant.grid(row=3, column=3, padx=20, pady=20)

        self.button_next_product = ctk.CTkButton(master=self.main_frame, text="siguiente", command=self.button_event_next_p)
        self.button_next_product.grid(row=4, column=3, padx=20, pady=20)
        
    def button_event_next_p(self):
        codigo = self.entry_code_products.get()
        nombre = self.entry_name_products.get()
        precio_dolar = self.entry_dolar_products.get()
        precio_bolivar = self.entry_price_products.get()
        descripcion = self.textbox_des.get("1.0", tk.END)
        cant = self.entry_cant.get()

        # Validar que el campo de teléfono contenga solo números
        if not precio_bolivar.replace(".","").isdigit() and not precio_dolar.replace(".","").isdigit():
            tkinter.messagebox.showerror("Error", "El campo de precio debe contener solo números")
            return

        # Validar que el campo de teléfono contenga solo números
        if not cant.isdigit():
            tkinter.messagebox.showerror("Error", "El campo de cantidad debe contener solo números")
            return
        ...
        try:
            ...
            conn = sqlite3.connect("ferretodo.db")
            c = conn.cursor()
            c.execute("INSERT INTO productos (codigo, nombre, precio_dolar, precio_bolivar, descripcion, cant) VALUES (?, ?, ?, ?, ?, ?)",
                      (codigo, nombre, precio_dolar, precio_bolivar, descripcion, cant))
            conn.commit()

            tkinter.messagebox.showinfo("Éxito", "El producto se ha creado exitosamente")
            # Cerrar la conexión a la base de datos
            c.close()
            conn.close()
        except Exception as e:
            tkinter.messagebox.showerror("Error", f"No se pudo agregar el producto: {e}")
